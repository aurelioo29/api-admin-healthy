...
const AboutUsCore = require("../models/aboutUs_core");
const AboutUsCoreI18n = require("../models/aboutUs_core_i18n");
...
const getListAboutUsCore = async (req, res, next) => {
  try {
    ...
    res.status(200).json({
      code: 200,
      success: true,
      message: "About Us Core retrieved successfully",
      data: {
        sections,
        total,
...
const createAboutUsCore = async (req, res, next) => {
  ...
  try {
    ...
    const section = await AboutUsCore.create(payload, { transaction: t });
    ...
    await AboutUsCoreI18n.bulkCreate(i18nRows, {
    ...
    await logActivity({
      userId: req.user.id,
      action: "CREATE",
      resource: "/upload/about-us-core",
      resourceId: section.id,
      description: `Create about us core section ${section.id}`,
    });
    ...
    return res.status(201).json({
      code: 201,
      success: true,
      message: "About Us Core created successfully",
      data: created,
    });
  } catch (error) {
    await t.rollback();
...
const updateAboutUsCore = async (req, res, next) => {
  const t = await sequelize.transaction();
  try {
    const id = Number(req.params.id);
    const section = await AboutUsCore.findByPk(id, { transaction: t });
    if (!section) {
      await t.rollback();
      return res.status(404).json({
        code: 404,
        success: false,
        message: "About Us Core not found",
      });
    }
    ...
    if (Object.keys(patch).length) {
      await AboutUsCore.update(patch, { where: { id }, transaction: t });
    }
    ...
    if (parsedI18n.length > 0) {
      ...
      await AboutUsCoreI18n.bulkCreate(rows, {
    ...
    await logActivity({
      ...
      resource: "/upload/about-us-core",
      resourceId: id,
      description: `Update about us core section ${id}`,
    });

    // Ambil ulang untuk response yang lebih informatif (opsional)
    const updated = await AboutUsCore.findByPk(id, {
      include: [
        {
          model: AboutUsCoreI18n,
          as: "i18n",
...
const deleteAboutUsCore = async (req, res, next) => {
  try {
    const { id } = req.params;
    const section = await AboutUsCore.findByPk(id);
    if (!section)
      return res
        .status(404)
    ...
    await AboutUsCoreI18n.destroy({ where: { section_id: id } });
    ...
    await logActivity({
      ...
      resource: "/upload/about-us-core",
      resourceId: id,
      description: `Delete about us core ${id}`,
      ipAddress: req.ip,
    ...
    res.status(200).json({
      code: 200,
      success: true,
      message: "About Us Core deleted successfully",
    });
  } catch (e) {
    next(e);
  }
