...
const AboutUsI18n = require("../models/aboutUs_i18n");
...
const getListAboutUs = async (req, res, next) => {
  try {
    ...
    const { count: total, rows } = await AboutUs.findAndCountAll({
      where,
      include: [
        includeI18n,
...
const createAboutUs = async (req, res, next) => {
  ...
  try {
    ...
    const section = await AboutUs.create(payload, { transaction: t });
    ...
    await AboutUsI18n.bulkCreate(i18nRows, {
    ...
    await logActivity({
      userId: req.user.id,
      action: "CREATE",
      resource: "/upload/about-us",
      resourceId: section.id,
      description: `Create about section ${section.id}`,
    });
...
const updateAboutUs = async (req, res, next) => {
  ...
  try {
    ...
    const section = await AboutUs.findByPk(id, { transaction: t });
    if (!section) {
      await t.rollback();
    ...
    if (Object.keys(patch).length) {
      await AboutUs.update(patch, { where: { id }, transaction: t });
    }
    ...
    if (parsedI18n.length > 0) {
      ...
      await AboutUsI18n.bulkCreate(rows, {
    ...
    await logActivity({
      ...
      resource: "/upload/about-us",
      resourceId: id,
      description: `Update about section ${id}`,
    });

    // Ambil ulang untuk response yang lebih informatif (opsional)
    const updated = await AboutUs.findByPk(id, {
      include: [
        {
          model: AboutUsI18n,
          as: "i18n",
...
const deleteAboutUs = async (req, res, next) => {
  try {
    const { id } = req.params;
    const section = await AboutUs.findByPk(id);
    if (!section)
      return res
        .status(404)
    ...
    await AboutUsI18n.destroy({ where: { section_id: id } });
    await section.destroy();

    await logActivity({
      userId: req.user.id,
      action: "DELETE",
      resource: "/upload/about",
      resourceId: id,
      description: `Delete about section ${id}`,
      ipAddress: req.ip,
