...
const AboutUsPresident = require("../models/aboutUs_president");
const AboutUsPresidentI18n = require("../models/aboutUs_president_i18n");
...
const getListAboutUsPresident = async (req, res, next) => {
  try {
    ...
    const includeI18n = {
      model: AboutUsPresidentI18n,
      as: "i18n",
      where: Object.keys(i18nWhere).length ? i18nWhere : undefined,
      required: !!search || isOneLocale,
    };

    const { count: total, rows } = await AboutUsPresident.findAndCountAll({
      where,
      include: [
        includeI18n,
    ...
    res.status(200).json({
      code: 200,
      success: true,
      message: "About Us President retrieved successfully",
      data: {
        sections,
        total,
...
const createAboutUsPresident = async (req, res, next) => {
  ...
  try {
    ...
    const section = await AboutUsPresident.create(payload, { transaction: t });
    ...
    await AboutUsPresidentI18n.bulkCreate(i18nRows, {
    ...
    await logActivity({
      userId: req.user.id,
      action: "CREATE",
      resource: "/upload/about-us-president",
      resourceId: section.id,
      description: `Create about us president section ${section.id}`,
    });
    ...
    return res.status(201).json({
      code: 201,
      success: true,
      message: "About Us President created successfully",
      data: created,
    });
  } catch (error) {
    await t.rollback();
...
const updateAboutUsPresident = async (req, res, next) => {
  const t = await sequelize.transaction();
  try {
    const id = Number(req.params.id);
    const section = await AboutUsPresident.findByPk(id, { transaction: t });
    if (!section) {
      await t.rollback();
      return res.status(404).json({
        code: 404,
        success: false,
        message: "About Us President not found",
      });
    }
    ...
    if (Object.keys(patch).length) {
      await AboutUsPresident.update(patch, { where: { id }, transaction: t });
    }
    ...
    if (parsedI18n.length > 0) {
      ...
      await AboutUsPresidentI18n.bulkCreate(rows, {
      ...
      if (loc === "id" || loc === "en") {
        await AboutUsPresidentI18n.upsert(
          {
            section_id: id,
            locale: loc,
    ...
    await logActivity({
      ...
      resource: "/upload/about-us-president",
      resourceId: id,
      description: `Update about us president section ${id}`,
    });

    // Ambil ulang untuk response yang lebih informatif (opsional)
    const updated = await AboutUsPresident.findByPk(id, {
      include: [
        {
          model: AboutUsPresidentI18n,
          as: "i18n",
...
const deleteAboutUsPresident = async (req, res, next) => {
  try {
    const { id } = req.params;
    const section = await AboutUsPresident.findByPk(id);
    if (!section)
      return res
        .status(404)
    ...
    await AboutUsPresidentI18n.destroy({ where: { section_id: id } });
    ...
    await logActivity({
      ...
      resource: "/upload/about-us-president",
      resourceId: id,
      description: `Delete about us president ${id}`,
      ipAddress: req.ip,
    ...
    res.status(200).json({
      code: 200,
      success: true,
      message: "About Us President deleted successfully",
    });
  } catch (e) {
    next(e);
  }
...
module.exports = {
  createAboutUsPresident,
  getListAboutUsPresident,
  updateAboutUsPresident,
  deleteAboutUsPresident,
};
